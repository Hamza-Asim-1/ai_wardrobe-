...
class FashionCLIPEncoder:
    ...
    def initialize(self):
        ...
        try:
            ...
            print(f"⚠ Fashion-CLIP unavailable ({e}). Using fallback embeddings.")
    ...
    def _encode_image_fallback(self, image_path: str) -> np.ndarray:
        """Create deterministic image embeddings without transformer weights."""
        from PIL import Image

        image = Image.open(image_path).convert("RGB").resize((32, 32))
        arr = np.asarray(image, dtype=np.float32) / 255.0
        stats = np.concatenate(
            [arr.mean(axis=(0, 1)), arr.std(axis=(0, 1)), arr.flatten()[:58]]
        )
        vec = np.zeros(self.config.embedding_dimension, dtype=np.float32)
        vec[: len(stats)] = stats[: self.config.embedding_dimension]
        norm = np.linalg.norm(vec)
        return vec if norm == 0 else vec / norm

    def _encode_text_fallback(self, text: str) -> np.ndarray:
        """Create deterministic text embeddings without transformer weights."""
        digest = hashlib.sha256(text.encode("utf-8")).digest()
        raw = np.frombuffer(digest, dtype=np.uint8).astype(np.float32)
        repeats = int(np.ceil(self.config.embedding_dimension / raw.size))
        vec = np.tile(raw, repeats)[: self.config.embedding_dimension]
