...
class YOLODetector:
    ...
    def initialize(self):
        ...
        try:
            ...
            print("⚠ Ultralytics unavailable. Falling back to full-image detection.")
            self.fallback_mode = True
        except Exception as e:
            print(f"⚠ YOLO initialization failed ({e}). Falling back to full-image detection.")
            self.fallback_mode = True
    ...
    def _fallback_detection(self, image_path: str) -> DetectionResult:
        """Fallback detector that treats whole image as one fashion item."""
        image = Image.open(image_path).convert("RGB")
